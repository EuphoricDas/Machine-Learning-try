...
import { videoflo_logo_64px } from '../../../shared/images';
...
export class RoomConfigComponent implements OnInit, OnDestroy {
  ...
  videoflo_logo_64px = videoflo_logo_64px;
  ...
  async onCameraSelected(event: any) {
    ...
    if (videoSource) {
      ...
      if (this.oVDevicesService.needUpdateVideoTrack(videoSource)) {
        ...
        await this.openViduWebRTCService.replaceTrack(
          videoSource,
          null,
          mirror
        );
  ...
  async onMicrophoneSelected(event: any) {
    ...
    if (audioSource) {
      ...
      if (this.oVDevicesService.needUpdateAudioTrack(audioSource)) {
        const mirror = this.oVDevicesService.cameraNeedsMirror(
  ...
  initNicknameAndSubscribeToChanges() {
    if (this.externalConfig) {
      this.localUsersService.updateUsersNickname(
        this.externalConfig.getNickname()
      );
      this.localUsersService.updateUsersNickname(
        this.externalConfig.getNickname()
      );
    ...
    const nickname =
      this.storageSrv.get(Storage.USER_NICKNAME) ||
      this.utilsSrv.generateNickname();
  ...
  private scrollToBottom(): void {
    try {
      this.bodyCard.nativeElement.scrollTop =
        this.bodyCard.nativeElement.scrollHeight;
    } catch (err) {}
  }
  ...
  private initScreenPublisher(): Publisher {
    ...
    const willThereBeWebcam =
      ...
      this.localUsersService.hasWebcamVideoActive();
  ...
  private subscribeToLocalUsersEvents() {
    this.oVUsersSubscription = this.localUsersService.OVUsers.subscribe(
      (users) => {
        this.localUsers = users;
      }
    );
    this.screenShareStateSubscription =
      this.localUsersService.screenShareState.subscribe((enabled) => {
  ...
  private async initwebcamPublisher() {
    ...
    const publisher = await this.openViduWebRTCService.initPublisherAsync(
      undefined,
      properties
    );
