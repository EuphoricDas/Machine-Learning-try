...
export class DevicesService {
  ...
  async initDevices(selectCamera = CameraType.FRONT) {
    await this.initOpenViduDevices();
    this.devices.length > 0
      ? this.log.d('Devices found: ', this.devices)
      : this.log.w('No devices found!');
    ...
    if (this.hasVideoDeviceAvailable()) {
      ...
      this.camSelected = this.cameras.find(
        (device) => device.type === selectCamera
      );
    }
  ...
  getCameras(): IDevice[] {
    return this.cameras.filter((val) => val.type !== null);
  }
