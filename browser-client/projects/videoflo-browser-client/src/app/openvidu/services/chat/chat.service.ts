...
export class ChatService {
  ...
  private _messageList = <BehaviorSubject<ChatMessage[]>>(
    new BehaviorSubject([])
  );
  ...
  subscribeToChat() {
    ...
    session.on('signal:chat', (event: any) => {
      ...
      const isMyOwnConnection =
        this.openViduWebRTCService.isMyOwnConnection(connectionId);
  ...
  sendMessage(message: string) {
    ...
    if (message !== '' && message !== ' ') {
      ...
      const sessionAvailable =
        this.openViduWebRTCService.getSessionOfUserConnected();
