...
export class RemoteUsersService {
  ...
  private _remoteUserNameList = <BehaviorSubject<UserName[]>>(
    new BehaviorSubject([])
  );
  ...
  add(event: StreamEvent | ConnectionEvent, subscriber: Subscriber) {
    ...
    let avatar = '';
    const connectionId =
      (<StreamEvent>event)?.stream?.connection?.connectionId ||
      (<ConnectionEvent>event)?.connection?.connectionId;
    const data = JSON.parse(
      (<StreamEvent>event)?.stream?.connection?.data ||
        (<ConnectionEvent>event)?.connection?.data
    );
    nickname = data?.name;
    avatar = this.avatarService.getAvatarFromConnectionData(data);
    const newUser = new UserModel(connectionId, subscriber, nickname, data);
  ...
  clear() {
    ...
    this.remoteUsers = this._remoteUsers.asObservable();
    this._remoteUserNameList = <BehaviorSubject<UserName[]>>(
      new BehaviorSubject([])
    );
    this.remoteUserNameList = this._remoteUserNameList.asObservable();
    this.users = [];
  ...
  deleteUserName(event: ConnectionEvent) {
    ...
    const newUserNameList: UserName[] = oldUserNameList.filter(
