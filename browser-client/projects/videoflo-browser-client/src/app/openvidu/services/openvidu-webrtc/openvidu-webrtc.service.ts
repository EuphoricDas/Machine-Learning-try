...
export class OpenViduWebrtcService implements IOpenViduWebRTC {
  ...
  constructor(
    private localUsersSrv: LocalUsersService,
  ...
  initPublisher(
    ...
    publisher.once('streamPlaying', () => {
      (<HTMLElement>publisher.videos[0].video).parentElement.classList.remove(
        'custom-class'
      );
    });
    return publisher;
  ...
  sendNicknameSignal(connection?: Connection) {
    if (this.needSendNicknameSignal()) {
      ...
      if (this.localUsersSrv.isScreenShareEnabled()) {
  ...
  needSendNicknameSignal(): boolean {
    const oldNickname: string = this.utilsSrv.getNicknameFromConnectionData(
      this.webcamSession.connection.data
    );
  ...
  getSessionOfUserConnected(): Session {
    return this.localUsersSrv.isWebCamEnabled()
      ? this.webcamSession
      : this.screenSession;
  }
